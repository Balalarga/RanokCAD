...
RanokCADApp::RanokCADApp()
    : Application(
          Params{
              .renderRate = 165,
              .updateRate = 100,
              .windowInitializer = Window::Params{
                  .Flags = Window::sWindowDefaultFlags | SDL_WINDOW_RESIZABLE
              }
          })
      , _activeModule(std::make_shared<DevelopmentModule>(GetWindow()->GetSize(), *GetInputManager()))
{
    SetDefaultStyle();
...
void RanokCADApp::RenderImGui()
{
    ...
    ImGui::SetNextWindowSize(ImVec2(GetWindow()->GetSize().x, GetWindow()->GetSize().y));
    if (_activeModule)
