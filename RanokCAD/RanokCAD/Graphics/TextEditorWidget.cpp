...
void TextEditorWidget::SetLanguageDefinition(const LanguageDefinition& aLanguageDef)
{
...
void TextEditorWidget::SetPalette(const Palette& aValue)
{
	mPaletteBase = aValue;
}

const TextEditorWidget::Palette& TextEditorWidget::GetGruvboxPalette()
{
	const static Palette p = {{
		ImGui::ColorConvertFloat4ToU32(ImVec4(234 / 255.f, 219 / 255.f, 178 / 255.f, 255 / 255.f)), // Default (FG)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(254 / 255.f, 128 / 255.f, 25 / 255.f, 255 / 255.f)), // Keyword (Light Orange)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(211 / 255.f, 133 / 255.f, 155 / 255.f, 255 / 255.f)), // Number (Light Purple)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(184 / 255.f, 187 / 255.f, 38 / 255.f, 255 / 255.f)),						// String (Light Green)
		ImGui::ColorConvertFloat4ToU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0 / 255.f)), // Char literal (none)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(234 / 255.f, 219 / 255.f, 178 / 255.f, 255 / 255.f)),					// Punctuation (Default)
		ImGui::ColorConvertFloat4ToU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0 / 255.f)), // Preprocessor (none)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(234 / 255.f, 219 / 255.f, 178 / 255.f, 255 / 255.f)), // Identifier (Light Yellow)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(184 / 255.f, 187 / 255.f, 38 / 255.f, 255 / 255.f)),						// Known identifier (same)
		ImGui::ColorConvertFloat4ToU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0 / 255.f)), // Preproc identifier (none)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(102 / 255.f, 92 / 255.f, 83 / 255.f, 255 / 255.f)), // Comment (single line) (bg2)
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(102 / 255.f, 92 / 255.f, 83 / 255.f, 255 / 255.f)), // Comment (multi line) (same)
		...
		ImGui::ColorConvertFloat4ToU32(
			ImVec4(204 / 255.f, 35 / 255.f, 28 / 255.f, 128 / 255.f)),						// ErrorMarker (Dark Red)
...
std::string TextEditorWidget::GetText(const Coordinates& aStart, const Coordinates& aEnd) const
{
	std::string result;
...
TextEditorWidget::Coordinates TextEditorWidget::GetActualCursorCoordinates() const
{
	return SanitizeCoordinates(mState.mCursorPosition);
}

TextEditorWidget::Coordinates TextEditorWidget::SanitizeCoordinates(const Coordinates& aValue) const
{
...
// code)
static int UTF8CharLength(TextEditorWidget::Char c)
{
	if ((c & 0xFE) == 0xFC)
		return 6;
	if ((c & 0xFC) == 0xF8)
...
void TextEditorWidget::Advance(Coordinates& aCoordinates) const
{
...
void TextEditorWidget::DeleteRange(const Coordinates& aStart, const Coordinates& aEnd)
{
	assert(aEnd >= aStart);
	assert(!mReadOnly);
...
int TextEditorWidget::InsertTextAt(Coordinates& /* inout */ aWhere, const char* aValue)
{
	assert(!mReadOnly);
...
void TextEditorWidget::AddUndo(UndoRecord& aValue)
{
	assert(!mReadOnly);
...
TextEditorWidget::Coordinates TextEditorWidget::ScreenPosToCoordinates(const ImVec2& aPosition) const
{
...
TextEditorWidget::Coordinates TextEditorWidget::FindWordStart(const Coordinates& aFrom) const
{
...
TextEditorWidget::Coordinates TextEditorWidget::FindWordEnd(const Coordinates& aFrom) const
{
	Coordinates at = aFrom;
	if (at.mLine >= (int)mLines.size())
		return at;
...
TextEditorWidget::Coordinates TextEditorWidget::FindNextWord(const Coordinates& aFrom) const
{
	Coordinates at = aFrom;
	if (at.mLine >= (int)mLines.size())
		return at;
...
int TextEditorWidget::GetCharacterIndex(const Coordinates& aCoordinates) const
{
...
int TextEditorWidget::GetCharacterColumn(int aLine, int aIndex) const
{
...
int TextEditorWidget::GetLineCharacterCount(int aLine) const
{
	if (aLine >= mLines.size())
		return 0;
	auto& line = mLines[aLine];
...
int TextEditorWidget::GetLineMaxColumn(int aLine) const
{
	if (aLine >= mLines.size())
		return 0;
	auto& line = mLines[aLine];
...
bool TextEditorWidget::IsOnWordBoundary(const Coordinates& aAt) const
{
...
void TextEditorWidget::RemoveLine(int aStart, int aEnd)
{
	assert(!mReadOnly);
	assert(aEnd >= aStart);
...
void TextEditorWidget::RemoveLine(int aIndex)
{
	assert(!mReadOnly);
...
TextEditorWidget::Line& TextEditorWidget::InsertLine(int aIndex)
{
	assert(!mReadOnly);
...
std::string TextEditorWidget::GetWordUnderCursor() const
{
	auto c = GetCursorPosition();
	return GetWordAt(c);
}

std::string TextEditorWidget::GetWordAt(const Coordinates& aCoords) const
{
...
ImU32 TextEditorWidget::GetGlyphColor(const Glyph& aGlyph) const
{
	if (!mColorizerEnabled)
...
void TextEditorWidget::HandleKeyboardInputs()
{
...
void TextEditorWidget::HandleMouseInputs()
{
	ImGuiIO& io = ImGui::GetIO();
	auto shift = io.KeyShift;
	auto ctrl = io.ConfigMacOSXBehaviors ? io.KeySuper : io.KeyCtrl;
	...
	if (ImGui::IsWindowHovered())
	{
		if (!shift && !alt)
		{
			...
			auto tripleClick =
				click && !doubleClick && (mLastClick != -1.0f && (t - mLastClick) < io.MouseDoubleClickTime);
...
void TextEditorWidget::Render(float fontScale)
{
	...
	auto lineMax =
		std::max(0, std::min((int)mLines.size() - 1, lineNo + (int)floor((scrollY + contentSize.y) / mCharAdvance.y)));
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			longest =
				std::max(mTextStart + TextDistanceToLineStart(Coordinates(lineNo, GetLineMaxColumn(lineNo))), longest);
			auto columnNo = 0;
			...
			if (mState.mSelectionStart <= lineEndCoord)
				sstart =
					mState.mSelectionStart > lineStartCoord ? TextDistanceToLineStart(mState.mSelectionStart) : 0.0f;
			if (mState.mSelectionEnd > lineStartCoord)
				ssend =
					TextDistanceToLineStart(mState.mSelectionEnd < lineEndCoord ? mState.mSelectionEnd : lineEndCoord);
				...
				if (focused)
				{
					...
					if (elapsed > 400)
					{
						...
						if (mOverwrite && cindex < (int)line.size())
						{
							...
							if (c == '\t')
							{
								auto x = (1.0f + std::floor((1.0f + cx) / (float(mTabSize) * spaceSize))) *
									(float(mTabSize) * spaceSize);
								width = x - cx;
							}
							else
...
void TextEditorWidget::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
...
void TextEditorWidget::SetText(const std::string& aText)
{
	mLines.clear();
...
void TextEditorWidget::SetTextLines(const std::vector<std::string>& aLines)
{
	mLines.clear();
...
void TextEditorWidget::EnterCharacter(ImWchar aChar, bool aShift)
{
	assert(!mReadOnly);
	...
	if (HasSelection())
	{
		if (aChar == '\t' && mState.mSelectionStart.mLine != mState.mSelectionEnd.mLine)
		{
			...
			for (int i = start.mLine; i <= end.mLine; i++)
			{
				...
				{
					line.insert(line.begin(), Glyph('\t', TextEditorWidget::PaletteIndex::Background));
					modified = true;
				}
			}
...
void TextEditorWidget::SetReadOnly(bool aValue)
{
	mReadOnly = aValue;
}

void TextEditorWidget::SetColorizerEnable(bool aValue)
{
	mColorizerEnabled = aValue;
}

void TextEditorWidget::SetCursorPosition(const Coordinates& aPosition)
{
...
void TextEditorWidget::SetSelectionStart(const Coordinates& aPosition)
{
...
void TextEditorWidget::SetSelectionEnd(const Coordinates& aPosition)
{
...
void TextEditorWidget::SetSelection(const Coordinates& aStart, const Coordinates& aEnd, SelectionMode aMode)
{
	...
	switch (aMode)
	{
		case TextEditorWidget::SelectionMode::Normal: break;
		case TextEditorWidget::SelectionMode::Word:
		{
		...
		case TextEditorWidget::SelectionMode::Line:
		{
...
void TextEditorWidget::SetTabSize(int aValue)
{
	mTabSize = std::max(0, std::min(32, aValue));
}

void TextEditorWidget::InsertText(const std::string& aValue)
{
	InsertText(aValue.c_str());
}

void TextEditorWidget::InsertText(const char* aValue)
{
	if (aValue == nullptr)
		return;
...
void TextEditorWidget::MoveUp(int aAmount, bool aSelect)
{
...
void TextEditorWidget::MoveDown(int aAmount, bool aSelect)
{
...
void TextEditorWidget::MoveLeft(int aAmount, bool aSelect, bool aWordMode)
{
	if (mLines.empty())
		return;
	...
	if (aSelect)
	{
		...
		mInteractiveStart = mInteractiveEnd = mState.mCursorPosition;
	SetSelection(
		mInteractiveStart, mInteractiveEnd, aSelect && aWordMode ? SelectionMode::Word : SelectionMode::Normal);

	EnsureCursorVisible();
}

void TextEditorWidget::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	auto oldPos = mState.mCursorPosition;
...
void TextEditorWidget::MoveTop(bool aSelect)
{
	auto oldPos = mState.mCursorPosition;
...
void TextEditorWidget::TextEditorWidget::MoveBottom(bool aSelect)
{
...
void TextEditorWidget::MoveHome(bool aSelect)
{
	auto oldPos = mState.mCursorPosition;
...
void TextEditorWidget::MoveEnd(bool aSelect)
{
	auto oldPos = mState.mCursorPosition;
...
bool TextEditorWidget::HasSelection() const
{
...
void TextEditorWidget::Undo(int aSteps)
{
	while (CanUndo() && aSteps-- > 0)
		mUndoBuffer[--mUndoIndex].Undo(this);
}

void TextEditorWidget::Redo(int aSteps)
{
	while (CanRedo() && aSteps-- > 0)
		mUndoBuffer[mUndoIndex++].Redo(this);
}

const TextEditorWidget::Palette& TextEditorWidget::GetDarkPalette()
{
	const static Palette p = {{
		0xff7f7f7f, // Default
		0xffd69c56, // Keyword
...
const TextEditorWidget::Palette& TextEditorWidget::GetLightPalette()
{
	const static Palette p = {{
		0xff7f7f7f, // None
		0xffff0c06, // Keyword
...
const TextEditorWidget::Palette& TextEditorWidget::GetRetroBluePalette()
{
	const static Palette p = {{
		0xff00ffff, // None
		0xffffff00, // Keyword
...
std::vector<std::string> TextEditorWidget::GetTextLines() const
{
...
std::string TextEditorWidget::GetSelectedText() const
{
	return GetText(mState.mSelectionStart, mState.mSelectionEnd);
}

std::string TextEditorWidget::GetCurrentLineText() const
{
...
void TextEditorWidget::ProcessInputs()
{
}

void TextEditorWidget::Colorize(int aFromLine, int aLines)
{
...
void TextEditorWidget::ColorizeRange(int aFromLine, int aToLine)
{
...
void TextEditorWidget::ColorizeInternal()
{
				...
				else
				{
					...
					else
					{
						...
						if (singleStartStr.size() > 0 && currentIndex + singleStartStr.size() <= line.size() &&
							equals(
								singleStartStr.begin(), singleStartStr.end(), from, from + singleStartStr.size(), pred))
						{
...
float TextEditorWidget::TextDistanceToLineStart(const Coordinates& aFrom) const
{
	...
	for (size_t it = 0u; it < line.size() && it < colIndex;)
	{
		...
		{
			distance =
				(1.0f + std::floor((1.0f + distance) / (float(mTabSize) * spaceSize))) * (float(mTabSize) * spaceSize);
			++it;
		}
		else
		{
...
int TextEditorWidget::GetPageSize() const
{
	auto height = ImGui::GetWindowHeight() - 20.0f;
	return (int)floor(height / mCharAdvance.y);
}

TextEditorWidget::UndoRecord::UndoRecord(const std::string& aAdded,
										 const TextEditorWidget::Coordinates aAddedStart,
										 const TextEditorWidget::Coordinates aAddedEnd,
										 const std::string& aRemoved,
										 const TextEditorWidget::Coordinates aRemovedStart,
										 const TextEditorWidget::Coordinates aRemovedEnd,
										 TextEditorWidget::EditorState& aBefore,
										 TextEditorWidget::EditorState& aAfter) :
	mAdded(aAdded),
	mAddedStart(aAddedStart), mAddedEnd(aAddedEnd), mRemoved(aRemoved), mRemovedStart(aRemovedStart),
...
void TextEditorWidget::UndoRecord::Undo(TextEditorWidget* aEditor)
{
	if (!mAdded.empty())
	{
...
void TextEditorWidget::UndoRecord::Redo(TextEditorWidget* aEditor)
{
	if (!mRemoved.empty())
	{
...
static bool
TokenizeCStyleCharacterLiteral(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;
...
static bool
TokenizeCStyleIdentifier(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	const char* p = in_begin;
...
static bool
TokenizeCStylePunctuation(const char* in_begin, const char* in_end, const char*& out_begin, const char*& out_end)
{
	(void)in_end;
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::RanokLanguageDefinition()
{
	...
	if (!inited)
	{
		...
		for (const std::string& k : Parser::GetReservedKeywords())
			langDef.mKeywords.insert(k);

		std::vector<std::pair<std::string, std::string>> identifiers;
		for (const std::string& name : Parser::GetReservedFuncwords())
			identifiers.emplace_back("Base function", name);

		for (auto& name : Hardcoded::Get().FunctionNames | std::views::values)
			identifiers.emplace_back("Base function", name);

		for (auto& name : Hardcoded::Get().VariableNames | std::views::values)
			identifiers.emplace_back("Base variable", name);
		...
		// {std::format("{}\n\n{}", libName, Ranok::FunctionDeclarationNode::GetDescription(func.second)),
		// func.first});
		// }
		// }
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::CPlusPlus()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		constexpr auto cppKeywords = {"alignas",
									  "alignof",
									  "and",
									  "and_eq",
									  "asm",
		...
		langDef.mTokenize = [](const char* in_begin,
							   const char* in_end,
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::HLSL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::GLSL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::C()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::SQL()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::AngelScript()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		static const char* const keywords[] = {
			...
			"continue", "default",	 "do",		"double",	"else",		"enum",	   "false",		"final",  "float",
			"for",		"from",		 "funcdef", "function", "get",		"if",	   "import",	"in",	  "inout",
			"int",		"interface", "int8",	"int16",	"int32",	"int64",   "is",		"mixin",  "namespace",
			"not",		"null",		 "or",		"out",		"override", "private", "protected", "return", "set",
			"shared",	"super",	 "switch",	"this ",	"true",		"typedef", "uint",		"uint8",  "uint16",
		...
		static const char* const identifiers[] = {
			"cos",		   "sin",		  "tab",		 "acos",	   "asin",	   "atan",	  "atan2",	  "cosh",
...
const TextEditorWidget::LanguageDefinition& TextEditorWidget::LanguageDefinition::Lua()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
